...
const ProjectData = () => {
  ...
  const handleBid = async () => {
    ...
    if (!bidAmount || !estimatedTime || !proposal.trim()) {
      setBidError('Please fill in all fields before placing a bid.')
      return
    }
    setBidding(true)
    try {
  ...
  // ✅ FIX: Allow bidding on both 'Available' and 'Pending' status
  const canBid     = project.status === 'Available' || project.status === 'Pending'
  ...
  return (
    <div className="pd pd-page">
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '32px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg,#1e3a8a,#2563eb,#4338ca)', padding: '24px 28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <div style={{ flexShrink: 0, marginLeft: 20,
                background: project.status === 'Available'
                  ? 'rgba(74,222,128,0.2)'
                ...
                color: project.status === 'Available'
                  ? '#86efac'
                  : project.status === 'Pending'
                  ? '#fde68a'
        ...
        {/* ── BID FORM ── ✅ FIXED: shows for both 'Available' and 'Pending' */}
        {canBid && (
            ...
            <div className="pd-card-body">
              ...
              {bidError && (
                <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12,
                  padding: '12px 16px', marginBottom: 16, fontSize: 13, fontWeight: 600, color: '#dc2626' }}>
                  {bidError}
                </div>
              )}
              ...
              <button
                className={alreadyBid ? undefined : 'pd-btn-green'}
